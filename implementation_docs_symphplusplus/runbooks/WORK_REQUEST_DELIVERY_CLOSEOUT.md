...
Use this runbook after a WorkRequest WorkPackage has been dispatched and the
...
3. If the dry-run proposes the expected PR-merged repair, apply
   `reconcile_work_request(apply: true)` or record the same
...
4. For no-PR completion, supersession, or abandonment, call
   `record_work_package_delivery` directly with the required typed evidence.
...
```json
{
  ...
  "work_package_id": "wrs_docs_hygiene",
...
{
  "work_request_id": "wr_kraken_docs",
  "work_package_id": "wrs_broad_docs",
  "outcome": "superseded",
  "idempotency_key": "kraken-broad-docs-recut",
  "evidence": {
    "superseded": {
      "successor_work_package_id": "wrs_narrow_docs",
...
- WorkPackage: raw status `dispatched`.
...
- `read_delivery_board` preserves the canonical package raw status.
- A stale package raw status remains visible as `ready_for_worker`.
- If merged-PR evidence is present, the package reports `Needs Closeout` rather
  than pretending the stale package card is still the human next action.

Expected projection after closeout:

- `record_work_package_delivery` records the outcome idempotently.
- `pr_merged` moves the canonical package to `merged`;
...
- WorkRequest completion refreshes when every WorkPackage has terminal
  delivery truth.
...
Merged-PR recovery may close a compatible stale package even if its raw
...
Supersession may close a compatible stale package even when active
...
keeps blocker attention on the terminal superseded package.
...
`cleanup_work_request_work_package_runtime(work_request_id, work_package_id,
outcome, reason, ...)` for the linked WorkPackage with the same superseded or
abandoned evidence that will be used inside `record_work_package_delivery`'s
typed `evidence` object, then rerun `record_work_package_delivery` with that
evidence. The cleanup tool
revokes live worker grants, releases non-paused current local claim
leases, clears recoverable worker MCP session bindings for the package,
...
`successor_work_package_id` and `superseded_reason`; `outcome=abandoned`
...
needed, `revoke_work_package_worker_key(work_request_id, work_package_id,
