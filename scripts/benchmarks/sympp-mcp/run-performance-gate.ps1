...
function Get-GateFailures($Metrics, $Limits) {
  ...
  if (-not $Metrics.exact.node.recovery.dashboard_healthy -or -not $Metrics.exact.node.mutation.checked -or -not $Metrics.exact.node.mutation.shortcut_rejected -or -not $Metrics.exact.node.mutation.scan_race_retried) { $failures.Add("exact.node.recovery_integrity") }
...
function Invoke-SelfTest {
  $base = [pscustomobject]@{
    ...
    exact = [pscustomobject]@{
      node = [pscustomobject]@{ cold = [pscustomobject]@{ trace = [pscustomobject]@{ installed_identity_full_validation = 1; payload_hash_validation = 1; marketplace_git_validation = 1; artifact_manifest_resolution = 1 } }; warm = @(New-SelfTestExactCohort 1; New-SelfTestExactCohort 10; New-SelfTestExactCohort 100); lock_recovery = [pscustomobject]@{ checked = $true; reclaimed = $true }; lifecycle_race = [pscustomobject]@{ checked = $true; healthy = $true }; recovery = [pscustomobject]@{ dashboard_healthy = $true }; mutation = [pscustomobject]@{ checked = $true; shortcut_rejected = $true; scan_race_retried = $true } }
