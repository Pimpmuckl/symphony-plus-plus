...
param(
  ...
  [ValidateRange(1, 60000)][int]$MaxExactWarmP95Ms = 2000,
  [ValidateRange(1, 60000)][int]$MaxExact100P95Ms = 10000,
  [ValidateRange(1, 600000)][int]$MaxExactLifecycleMs = 180000,
...
$exactP95Caps = @{ 1 = $MaxExactWarmP95Ms; 10 = $MaxExactWarmP95Ms; 100 = $MaxExact100P95Ms }
$thresholds = @{
  ...
  exact_warm_bytes = $MaxExactWarmBytes; exact_lifecycle_ms = $MaxExactLifecycleMs; direct_ms = $MaxDirectMs
...
function Get-GateFailures($Metrics, $Limits) {
  ...
  if ($node100.Count -ne 1 -or $node100[0].start_barrier -or [int]$node100[0].startup_burst -lt 1 -or [int]$node100[0].startup_burst -gt 10 -or [int]$node100[0].leases_peak -ne 101 -or [int]$node100[0].leases_after -ne 1 -or [int]$node100[0].server_leases_peak -ne 101 -or [int]$node100[0].process_tree.node_clients -ne 100 -or [int]$node100[0].process_tree.min_processes_per_client -lt 2) { $failures.Add("exact.node.live_100") }
  $lifecycle = $Metrics.exact.node.lifecycle_barrier
  if (-not $lifecycle.checked -or -not $lifecycle.start_barrier -or [int]$lifecycle.clients -ne 200 -or [int]$lifecycle.startup_burst -ne 200 -or
      [int]$lifecycle.leases_peak -ne 201 -or [int]$lifecycle.leases_after -ne 1 -or [int]$lifecycle.server_leases_peak -ne 201 -or
      [int]$lifecycle.process_tree.node_clients -ne 200 -or -not $lifecycle.identity_unchanged -or -not $lifecycle.anchor_protected -or
      [int]$lifecycle.trace.backend_recovery_ready -ne 0 -or [int]$lifecycle.trace.last_detach_cleanup_requested -ne 0) { $failures.Add("exact.node.lifecycle_boundary") }
  if ([double]$lifecycle.completion_ms -gt $Limits.exact_lifecycle_ms) { $failures.Add("exact.node.lifecycle_ms") }
...
function Write-Result($Metrics, [string[]]$Failures, $Cleanup) {
  ...
  [Console]::Out.WriteLine("  exact_200_lifecycle_ms: $MaxExactLifecycleMs")
  ...
  [Console]::Out.WriteLine("  node_200_lifecycle_ms: $($Metrics.exact.node.lifecycle_barrier.completion_ms)")
...
function New-SelfTestExactCohort([int]$Clients) {
  return [pscustomobject]@{
    ...
    leases_after = 1
    server_leases_peak = $Clients + 1
    git_invocations = 0
    trace = [pscustomobject]@{ payload_hash_validation = 0; marketplace_git_validation = 0; contract_fingerprint_resolution = 0; artifact_manifest_resolution = 0; backend_recovery_ready = 0; last_detach_cleanup_requested = 0 }
...
function Invoke-SelfTest {
  $base = [pscustomobject]@{
    ...
    exact = [pscustomobject]@{
      node = [pscustomobject]@{ artifact = [pscustomobject]@{ cache_miss = [pscustomobject]@{ process_ms = 1 }; prepared_cache = [pscustomobject]@{ process_ms = 1 } }; cold = [pscustomobject]@{ trace = [pscustomobject]@{ installed_identity_full_validation = 1; payload_hash_validation = 0; marketplace_git_validation = 0; artifact_manifest_resolution = 1 } }; warm = @(New-SelfTestExactCohort 1; New-SelfTestExactCohort 10; New-SelfTestExactCohort 100); lifecycle_barrier = [pscustomobject]@{ checked = $true; clients = 200; startup_burst = 200; start_barrier = $true; completion_ms = 1; leases_peak = 201; leases_after = 1; server_leases_peak = 201; identity_unchanged = $true; anchor_protected = $true; process_tree = [pscustomobject]@{ node_clients = 200 }; trace = [pscustomobject]@{ backend_recovery_ready = 0; last_detach_cleanup_requested = 0 } }; lock_recovery = [pscustomobject]@{ checked = $true; reclaimed = $true }; lifecycle_race = [pscustomobject]@{ checked = $true; healthy = $true }; recovery = [pscustomobject]@{ dashboard_healthy = $true }; mutation = [pscustomobject]@{ checked = $true; shortcut_rejected = $true; scan_race_detected = $true; attach_race_rejected = $true } }
  ...
  $cases = [ordered]@{
    ...
    "exact.node.lifecycle_boundary" = { param($m) $m.exact.node.lifecycle_barrier.trace.last_detach_cleanup_requested = 1 }
    "exact.node.lifecycle_ms" = { param($m) $m.exact.node.lifecycle_barrier.completion_ms = $MaxExactLifecycleMs + 1 }
