defmodule SymphonyElixir.SymphonyPlusPlus.MCP.ToolSurfaceLeanTest do
    ...
    for profile <- @profiles do
      ...
      case profile do
        ...
        profile when profile in [:coordinator, :solo] ->
          assert MapSet.equal?(names, MapSet.new(["sympp.health", "get_current_assignment", "release_current_assignment" | ToolCatalog.solo_tools()]))
      end
    end
  end
  ...
  test "full and default expose the stable composite catalog" do
    ...
    requested_full =
      ToolCatalog.startup_tool_specs(:full, Config.default(surface_profile: :worker, source_revision: nil))
