  ...
  test "refresh script prunes generated local cache and overlays manifest-version cache" do
    ...
    if powershell do
      for cache_name <- ["local", @plugin_version] do
        ...
        stale_legacy_solo_skill_path = published_plugin_cache_path(temp_codex_home, [cache_name, "skills-default", "symphony-solo-session", "SKILL.md"])
        ...
        File.mkdir_p!(Path.dirname(stale_legacy_solo_skill_path))
        File.write!(stale_legacy_solo_skill_path, "stale legacy default skill")
      ...
      try do
        ...
        for cache_name <- [@plugin_version] do
          ...
          assert File.exists?(published_plugin_cache_path(temp_codex_home, [cache_name, "skills", "symphony-solo-session", "SKILL.md"]))
          assert File.exists?(published_plugin_cache_path(temp_codex_home, [cache_name, "skills", "symphony-worker", "SKILL.md"]))
          assert File.exists?(published_plugin_cache_path(temp_codex_home, [cache_name, "skills", "symphony-coordinator", "SKILL.md"]))
          refute File.exists?(published_plugin_cache_path(temp_codex_home, [cache_name, "skills", "symphony-architect"]))
          refute File.exists?(published_plugin_cache_path(temp_codex_home, [cache_name, "skills-default"]))
