defmodule SymphonyElixir.SymphonyPlusPlus.MCP.Server do
  ...
  defp architect_tool("reconcile_work_request", arguments, %__MODULE__{config: config, session: session}),
    do: ArchitectDeliveryTools.call("reconcile_work_request", arguments, config, session)

  defp architect_tool("record_planned_slice_delivery", arguments, %__MODULE__{config: config, session: session}),
    do: ArchitectDeliveryTools.call("record_planned_slice_delivery", arguments, config, session)
  ...
  defp architect_tool(name, arguments, %__MODULE__{} = server) when name in ["cleanup_work_request_planned_slice_runtime", "revoke_planned_slice_worker_key"],
    do: ArchitectDeliveryTools.call(name, arguments, server.config, server.session)
  ...
  defp architect_tool("ask_work_request_question", arguments, %__MODULE__{config: config, session: session}) do
    with {:ok, session} <- Auth.require_session(session, config.repo),
         ...
         {:ok, _work_request, filters, scope} <-
           WorkRequestScope.authorized_work_request_scope(config.repo, session, work_request_id, :question_create, "ask_work_request_question"),
         {:ok, question_record} <-
  ...
  defp architect_tool("prepare_work_package_worktree", arguments, %__MODULE__{config: config, session: session}) do
    with {:ok, session} <- architect_session(config.repo, session, "dispatch:work_request"),
         ...
         {:ok, audit_event} <- ArchitectDeliveryTools.append_worktree_lifecycle_audit(config.repo, session, work_package_id, "prepare_work_package_worktree", result) do
  ...
  defp architect_tool("cleanup_work_package_worktree", arguments, %__MODULE__{config: config, session: session}) do
    with {:ok, session} <- architect_session(config.repo, session, "dispatch:work_request"),
         ...
         {:ok, audit_event} <- ArchitectDeliveryTools.maybe_append_cleanup_worktree_audit(config.repo, session, work_package_id, result) do
      {:ok, ToolResult.tool_result(worktree_lifecycle_payload(result, scope, audit_event))}
    else
  ...
  defp upsert_product_plan_node_with_blocker_closeout(repo, %Session{} = session, attrs, blocker_closeout_plan) do
    with {:ok, product_tree_node} <- ProductTree.upsert_node(repo, attrs),
         {:ok, blocker_closeout} <- BlockerCloseout.apply(repo, session, blocker_closeout_plan) do
  ...
  defp maybe_prepare_work_package_status_blocker_closeout(repo, %Session{} = session, status, arguments)
    ...
    BlockerCloseout.prepare_scoped(repo, session, [Session.work_package_id(session)], arguments, "set_status")
  end
  ...
  defp maybe_prepare_product_plan_node_blocker_closeout(repo, %Session{} = session, work_request_id, product_tree_node_id, _completion_mark, arguments, tool) do
    with {:ok, work_package_ids} <- product_plan_node_work_package_ids(repo, work_request_id, product_tree_node_id) do
      BlockerCloseout.prepare_scoped(repo, session, work_package_ids, arguments, tool)
    end
  end
  ...
  defp worker_tool("mark_ready", arguments, %__MODULE__{config: config, session: session}) do
    with {:ok, session} <- Auth.require_session(session, config.repo),
         ...
         {:ok, blocker_closeout_plan} <- BlockerCloseout.prepare_scoped(config.repo, session, [Session.work_package_id(session)], arguments, "mark_ready"),
         {:ok, {work_package, blocker_closeout, warnings}} <-
           ReviewReadiness.mark_ready(config.repo, session, blocker_closeout_plan, review_suite_result, &BlockerCloseout.apply/3) do
      {:ok,
       ToolResult.tool_result(
