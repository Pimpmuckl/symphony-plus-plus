...
Assert-True (@($coldSmoke.recovery).Count -eq 6 -and @($coldSmoke.recovery | Where-Object { $_.mode -ne "shutdown_during_recovery" -and ($_.backends -ne 2 -or $_.pids -ne 2 -or $_.listeners -ne 0 -or $_.recovery_leaders -ne 1) }).Count -eq 0) "Owner loss, backend loss, ambiguous-tool recovery, and rejected replacement recovery must each elect exactly one replacement backend and still reach zero listeners"
