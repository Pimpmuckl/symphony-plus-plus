...
Assert-True ($node.IndexOf('cleanupScript = prepareCleanupScript(identity);') -lt $watchRelease -and $node.Contains('cleanupLastDetach(runtimeFile, current.identity.runtimeKey, cleanupScript)') -and -not $node.Contains('PreserveCurrentArtifactRuntime')) "Node warm attach must preserve last-detach cleanup for the current epoch outside the invalidatable plugin cache without a residency exception"
...
Assert-True (@($coldSmoke.recovery).Count -eq 7 -and @($coldSmoke.recovery | Where-Object { $_.mode -ne "shutdown_during_recovery" -and ($_.backends -ne 2 -or $_.pids -ne 2 -or $_.listeners -ne 0 -or $_.recovery_leaders -ne 1) }).Count -eq 0) "Node and PowerShell fallback recovery must each elect exactly one replacement backend and still reach zero listeners"
Assert-True (($coldSmoke.recovery | Where-Object mode -eq "ambiguous_tool").mutations -eq 1 -and @($coldSmoke.recovery | Where-Object { $_.mode -notin "shutdown_during_recovery", "generation_changed_recovery", "cleanup_source_changed_recovery" -and $_.tools_list -le $_.clients }).Count -eq 0) "Recovered adapters must rebind tools/list and never replay the ambiguous mutating tool call"
...
Assert-True (($coldSmoke.recovery | Where-Object mode -eq "powershell_fallback_recovery").fallback_recovery) "Surviving PowerShell fallback adapters must retain STDIO, rebind the replacement, and drain it after final detach"
