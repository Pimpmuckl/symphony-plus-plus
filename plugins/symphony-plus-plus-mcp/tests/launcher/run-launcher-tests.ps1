...
Assert-True ([regex]::Matches($node, 'setTimeout\(resolve, GENERATION_SETTLE_MS\)').Count -eq 1) "Generation validation must retain only the final exact-generation settle wait"
